...
def update_games():
    ...
    while True:
        ...
        # try/catch block to handle TimeoutException in EC2
        try:
    ...
    for row in element.find_elements(By.CSS_SELECTOR, 'a'):
        inside_array = []
        endTime = time.time()
        print(f"Progress: "+"{0:.2%}. Time elapsed: {1}".format(i/totalFinds, timedelta(seconds=endTime-startTime)))
    ...
    games.to_sql(name='steam', con=engine, if_exists='replace', index=False)
